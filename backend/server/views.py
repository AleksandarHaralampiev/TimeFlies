...
def getTimeLine(request, *args, **kwargs):
    if request.method == "GET":
        try:
            ...
            servers_data = [{"id": server.id, "name": server.name, "description": server.description, "public": server.public, "owner_id": server.owner_id, "owner_username": UserAccount.objects.filter(id = server.owner_id).first().username} for server in servers]
...
def getAllPublicTimeLine(request, *args, **kwargs):
    if request.method == "GET":
        try:
            ...
            servers_data = [{"id": server.id, "name": server.name, "description": server.description, "public": server.public, "owner_id": server.owner_id, "owner_username": UserAccount.objects.filter(id = server.owner_id).first().username} for server in publicServers]
            return Response(data = {"servers": servers_data}, status=200)
        except:
            return Response(data={"error": "Invalid request"}, status=400)
